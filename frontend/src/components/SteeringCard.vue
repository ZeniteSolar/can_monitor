    ...
    <div class="steering-body px-2 pb-2 pt-1">
      <v-card-text class="d-flex justify-center pa-0">
        <svg :width="svgWidth" :height="svgHeight" :viewBox="`0 0 ${svgWidth} ${svgHeight}`"
          ...
          <mask id="rectMask">
            <rect :x="cx - radius - 70" y="0" :width="radius * 2 + 140" height="140" fill="white" />
            <circle :cx="cx" cy="0" :r="radius" fill="black" />
          </mask>

          <rect :x="cx - radius - 70" y="0" :width="radius * 2 + 140" height="140" rx="10" ry="10" fill="orange"
            stroke="black" stroke-width="2" mask="url(#rectMask)" />

          <circle :cx="cx" cy="0" :r="radius" fill="none" stroke="black" stroke-width="2" />


          <!-- Boat label -->
          <rect :x="cx - 50" y="105" width="100" height="30" rx="5" ry="5" fill="white" stroke="black"
            stroke-width="1" />
          <text :x="cx" y="125" text-anchor="middle" fill="black" font-family="sans-serif" font-size="16">Guarapuvu
            II</text>


          <!-- Steering Pointer -->
          <polygon :points="pointerPoints(steeringAngle, radius)" fill="red" stroke="red" stroke-width="5"
            stroke-linejoin="round" />

          <!-- Tail Pointer -->
          <polygon :points="pointerPoints(tailAngle, radius)" fill="black" stroke="black" stroke-width="7"
            stroke-linejoin="round" />
        </svg>
      </v-card-text>
    </div>
...
const props = defineProps<{
  data: number[]; // [steering_raw, tail_raw]
  title: string;
  titleColor?: string;
}>();
...
const svgHeight = 150;
const radius = 100;
const cx = svgWidth / 2; // ✅ this centers everything
...
function toBottomSemicircleAngle(raw: number): number {
  const clamped = Math.max(0, Math.min(241, raw));
  return 180 + (clamped / 241) * 180;
}
