...
async fn main() -> Result<()> {
    ...
    let backend = tokio::spawn(run_backend(tx));

    let frontend = tokio::spawn(run_frontend(rx));
...
async fn run_frontend(mut rx: mpsc::Receiver<can_types::modules::Messages>) -> Result<()> {
    loop {
...
async fn run_backend(tx: mpsc::Sender<can_types::modules::Messages>) -> Result<()> {
    loop {
